...
class FastSignalGenerator:
    ...
    def analyze_multiple_timeframes(self, df_1m, df_5m, df_15m, df_1h):
        ...
        for timeframe, df in timeframes:
            if df is not None and len(df) > 10:  # Уменьшили минимальное количество свечей
    ...
    def analyze_single_timeframe(self, df, timeframe):
        ...
        try:
            ...
            if not self.is_nan(current['rsi']):
                max_score += 3
                if current['rsi'] < 40:  # Увеличили диапазон для больше сигналов
                    bullish_score += 3
                elif current['rsi'] > 60:
                    bearish_score += 3
            ...
            # Сниженный порог для быстрых сигналов
            min_confidence = 35  # Сильно снизили порог
    ...
    def generate_final_signal(self, timeframe_signals, symbol):
        ...
        recent_signals = [s for s in self.signal_history[symbol] 
                         if current_time - s['timestamp'] < 60]  # 1 минута
        
        if recent_signals:
            return None  # Пропускаем если был сигнал в последнюю минуту
...
class MEXCFastOptionsBot:
    def __init__(self):
        ...
        self.config = {
            ...
            'min_candles': 10,  # Уменьшили минимальное количество свечей
    ...
    async def initialize_telegram(self):
        try:
            ...
            startup_message = (
                ...
                "• Низкие пороги для частых сигналов\n"
    ...
    async def handle_fast_trading(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        try:
            ...
            time_message += "• Используйте сигналы с уверенностью от 30%\n"
    ...
    async def handle_help(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        help_message = (
            ...
            "• Анализ каждые 30 секунд\n"
            "• Сигналы с уверенностью от 30%\n"
            "• Упрощенная логика для частых сигналов\n"
            "• Минимальные задержки"
        )
        await update.message.reply_text(
            help_message,
                ...
                if update:
                    if signals:
                        await update.message.reply_text(
                            ...
                            f"🎯 Минимальная уверенность: <b>30%</b>\n\n"
    ...
    async def analyze_symbol(self, symbol: str):
        ...
        try:
            ...
            confidences = [s.get('confidence', 0) for s in timeframe_signals.values() if s.get('signal') != 'NEUTRAL']
            avg_confidence = np.mean(confidences) if confidences else 50  # Базовая уверенность
    ...
    async def analyze_market(self):
        ...
        for symbol in self.target_symbols:
            try:
                ...
                if signal:  # Убрали проверку на уверенность для тестирования
        ...
        # Если нет сигналов, создаем тестовый сигнал для демонстрации
        if not signals and self.statistics['total_analyses'] % 3 == 0:
            logger.info("Создаю тестовый сигнал для демонстрации")
            entry_time, expiration_time, _ = self.calculate_optimal_times()
            test_signal = {
                'symbol': 'BTC/USDT',
    ...
    def print_signals(self):
        ...
        for i, signal in enumerate(self.signals):
            ...
            print(f"\n{i+1}. {symbol_name} | {signal['signal']} | Уверенность: {signal.get('confidence', 0):.1f}%")
    ...
    async def run_continuous(self):
        ...
        while True:
            try:
                ...
                print(f"🎯 Минимальная уверенность: 30%")
...
async def main():
    ...
    try:
        ...
        print("🎯 Минимальная уверенность сигналов: 30%")
        print("⏰ Таймфреймы: 1M, 5M, 15M, 1H")
        print("🔁 Анализ каждые 30 секунд")
        print("⚠️  Бот создает тестовые сигналы если нет реальных")
