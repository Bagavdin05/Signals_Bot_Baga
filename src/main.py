    ...
    async def send_automatic_signals(self):
        ...
        if not self.signals:
            ...
            no_signals_message = (
                ...
                f"⏱️ Время анализа: {html.escape(analysis_time_str)}\n"
                f"🌍 Часовой пояс: Москва (UTC+3)\n"
            )
    ...
    async def run_continuous(self):
        ...
        while True:
            try:
                ...
                next_analysis_time = time.time() + wait_time
                next_time_str = self.format_moscow_time(datetime.fromtimestamp(next_analysis_time), "%H:%M:%S")

                print(f"⏭️ Следующий анализ в {next_time_str} МСК (через {wait_time:.0f} секунд)")
